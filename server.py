...
async def _handle(msg: dict, creds, session_id: str) -> Optional[dict]:
    ...
    try:
        if method == "initialize":
            ...
            logger.info("[TOOL] calling %s — creds_present=%s", tool_name, creds is not None)
            try:
                output = await _dispatch(tool_name, params.get("arguments", {}), creds)
                result = {"content": [{"type": "text", "text": json.dumps(output, indent=2, ensure_ascii=False)}]}
            except Exception as tool_exc:
                logger.error("[TOOL] %s failed: %s\n%s", tool_name, tool_exc, traceback.format_exc())
                raise
        else:
            return {"jsonrpc": "2.0", "id": msg_id, "error": {"code": -32601, "message": f"Method not found: {method}"}}
        return {"jsonrpc": "2.0", "id": msg_id, "result": result}
    except Exception as exc:
        logger.error("[HANDLE] exception for method=%s: %s\n%s", method, exc, traceback.format_exc())
...
async def mcp_post(request: Request):
    ...
    if token_str:
        try:
            ...
            logger.info("[AUTH] token decrypted ok — user_id=%s has_refresh_token=%s", user_id, bool(refresh_token))
            if refresh_token:
                creds = await credentials_from_refresh_token(refresh_token, user_id)
                logger.info("[AUTH] credentials_from_refresh_token → creds=%s valid=%s expired=%s",
                            creds is not None,
                            getattr(creds, "valid", "?") if creds else "n/a",
                            getattr(creds, "expired", "?") if creds else "n/a")
        except Exception:
            logger.error("[AUTH] token decrypt failed:\n%s", traceback.format_exc())
